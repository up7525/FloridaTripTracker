...
public class RecordingService extends Service implements LocationListener {
	...
	// Alert ring tone bell variables
	...
	public void onCreate() {
        ...
        .setContentType(AudioAttributes.USAGE_NOTIFICATION_EVENT)
        .build();*/
	...
	public void remindUser() {
		...
		int icon = R.drawable.dot25;
        long when = System.currentTimeMillis();
        int minutes = (int) (when - trip.startTime) / 60000;
		CharSequence tickerText = String.format("Still recording (%d min)", minutes);

		//HS-Notification notification = new Notification(icon, tickerText, when);
		Notification notification = new Notification.Builder(getBaseContext()).setTicker(tickerText).setSmallIcon(icon).setWhen(when).build();
		//replace above with Notification notification = new Notification.Builder(context).setTicker(tickerText).setSmallIcon(icon).setWhen(when).build();
		notification.flags |=
		...
		CharSequence contentTitle = "TripTracks - Recording";
